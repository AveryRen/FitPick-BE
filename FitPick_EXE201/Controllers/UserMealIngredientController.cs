        ...
        public async Task<IActionResult> GetUserMealIngredients(int mealId)
        {
            try
            {
                ...
                return Ok(ApiResponse<List<UserMealIngredientDto>>
                    .SuccessResponse(data, "L?y danh sách nguyên li?u thành công"));

            }
            catch (Exception ex)
            {
                return BadRequest(ApiResponse<List<MealIngredientDto>>
                    .ErrorResponse(new List<string> { ex.Message }, "Không th? l?y danh sách nguyên li?u"));
            }
        }
        ...
        public async Task<IActionResult> MarkIngredient([FromBody] UserMealIngredientMarkDto dto)
        {
            try
            {
                ...
                return Ok(ApiResponse<object>
                    .SuccessResponse(null, "C?p nh?t nguyên li?u thành công"));
            }
            catch (Exception ex)
            {
                return BadRequest(ApiResponse<object>
                    .ErrorResponse(new List<string> { ex.Message }, "Không th? c?p nh?t nguyên li?u"));
            }
        }
    }
}
