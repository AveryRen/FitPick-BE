...
{
    public class UserBlogRepo : IUserBlogRepo
    {
        private readonly FitPickContext _context;

        public UserBlogRepo(FitPickContext context)
        {
            _context = context;
        }
        ...
        public async Task<Blogpost?> GetByIdAsync(int postId)
        {
            return await _context.Blogposts
                .Include(b => b.Author)
                .Include(b => b.Category)
                .Include(b => b.BlogMedia) // load luôn media
        ...
        public async Task<bool> UpdateAsync(Blogpost post, int currentUserId, string currentUserRole)
        {
            ...
            if (currentUserRole != "Admin" && existing.Authorid != currentUserId)
                return false;

            existing.Title = post.Title;
            existing.Content = post.Content;
            existing.Status = post.Status;
            existing.Categoryid = post.Categoryid;  // Cập nhật category
            post.Updatedat = DateTime.SpecifyKind(DateTime.UtcNow, DateTimeKind.Unspecified);
