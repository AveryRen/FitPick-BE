...
namespace FitPick_EXE201.Controllers
{
    ...
    public class AdminMealController : ControllerBase
    {
            ...
            [FromQuery] decimal? maxPrice)
        {
            ...
            return Ok(ApiResponse<IEnumerable<Meal>>.SuccessResponse(meals, "L?y danh sách meal thành công"));
        }
        ...
        public async Task<IActionResult> GetById(int id)
        {
            ...
            if (meal == null)
                return NotFound(ApiResponse<Meal>.ErrorResponse(
                    new List<string> { "Meal không t?n t?i" }, "Không tìm th?y meal"));

            return Ok(ApiResponse<Meal>.SuccessResponse(meal, "L?y meal thành công"));
        }
        ...
        public async Task<IActionResult> Create([FromBody] MealCreateDto dto)
        {
            if (!ModelState.IsValid)
            {
                ...
                return BadRequest(ApiResponse<Meal>.ErrorResponse(errors, "D? li?u không h?p l?"));
            }
            ...
            return Ok(ApiResponse<Meal>.SuccessResponse(createdMeal, "T?o meal thành công"));
        }
        ...
        public async Task<IActionResult> Update(int id, [FromBody] MealUpdateDto dto)
        {
            if (!ModelState.IsValid)
            {
                ...
                return BadRequest(ApiResponse<MealUpdateDto>.ErrorResponse(errors, "D? li?u không h?p l?"));
            }
            ...
            return Ok(ApiResponse<Meal>.SuccessResponse(updatedMeal, "C?p nh?t meal thành công"));
        }
        ...
        public async Task<IActionResult> UpdateMealImage(int id, [FromForm] MealImageUpdateDto dto)
        {
            if (!ModelState.IsValid || dto.File == null || dto.File.Length == 0)
            {
                return BadRequest(ApiResponse<string>.ErrorResponse(
                    new List<string> { "File ?nh không h?p l?" },
            ...
            {
                return NotFound(ApiResponse<string>.ErrorResponse(
                    new List<string> { "Meal không t?n t?i" },
                    "Không tìm th?y meal"
                ));
            } 
            ...
            return Ok(ApiResponse<MealImageResponseDto>.SuccessResponse(responseDto, "C?p nh?t ?nh món an thành công"));
        }
