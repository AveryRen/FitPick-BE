...
namespace FitPick_EXE201.Repositories.Repo
{
    public class MealPlanRepo : IMealPlanRepo
    {
        ...
        {
            // 1?? L?y user marks tru?c (tránh join tr?c ti?p trong LINQ to Entities)
            ...
            // 3?? Map thành DTO, load Instructions + Ingredients trong memory
        ...
        // Sinh meal plan m?i cho 1 ngày, tránh duplicate
        public async Task<List<Mealplan>> GenerateMealPlanAsync(int userId, DateOnly date)
        {
            // Xóa meal plan cu c?a user trong ngày (n?u có)
            ...
            // L?y meals phù h?p calories / goal
            ...
            {
                // Gi? s? m?i b?a có 2 món ng?u nhiên (có th? thay d?i s? lu?ng)
