...
namespace FitPick_EXE201.Controllers
{
    ...
    public class MealPlansController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<List<TodayMealPlanDto>>>> GetTodayMealPlan()
        {
            ...
            return Ok(ApiResponse<List<TodayMealPlanDto>>.SuccessResponse(plans, "L?y th?c don hôm nay thành công"));
        }
        ...
        public async Task<ActionResult<ApiResponse<List<Mealplan>>>> GetUserMealPlans()
        {
            ...
            return Ok(ApiResponse<List<Mealplan>>.SuccessResponse(plans, "L?y toàn b? meal plan thành công"));
        }
        ...
        public async Task<ActionResult<ApiResponse<Mealplan>>> GenerateMealPlan([FromQuery] DateTime date)
        {
            ...
            if (plan == null)
                return BadRequest(ApiResponse<Mealplan>.ErrorResponse(
                    new List<string> { "Không th? t?o meal plan" }, "Th?t b?i"));

            return Ok(ApiResponse<Mealplan>.SuccessResponse(plan, "T?o meal plan thành công"));
        }
        ...
        public async Task<ActionResult<ApiResponse<Mealplan>>> SwapMeal(int id, [FromQuery] int newMealId)
        {
            ...
            if (plan == null)
                return NotFound(ApiResponse<Mealplan>.ErrorResponse(
                    new List<string> { "Meal plan không t?n t?i" }, "Th?t b?i"));

            return Ok(ApiResponse<Mealplan>.SuccessResponse(plan, "Hoán d?i món thành công"));
        }
